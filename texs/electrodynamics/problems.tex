...
\begin{document}
	...
	\begin{center}
		...
		\begin{array}{c}
			...
			v_z \\
		\end{array}\right) = \left(\begin{array}{c}
			\frac{c - v_x \frac{V}{c}}{\sqrt{1-\frac{V^2}{c^2}}} \\
			\frac{v_x - V}{\sqrt{1-\frac{V^2}{c^2}}} \\
			v_y \\
			v_z \\
		\end{array}\right)$
	\end{center}
	...
	\begin{center}
		$v'_x = c \dfrac{v_x - V}{\sqrt{1-\frac{V^2}{c^2}}} \dfrac{\sqrt{1-\frac{V^2}{c^2}}}{c-v_x\frac{V}{c}} = \dfrac{v_x - V}{1 - \frac{v_x V}{c^2}}, \quad $
		$v'_y = c\dfrac{v_y\sqrt{1-\frac{V^2}{c^2}}}{c-v_x \frac{V}{c}} = \dfrac{v_y\sqrt{1 - \frac{V^2}{c^2}}}{1 - \frac{v_x V}{c^2}}$
		
		$v'_z = c\dfrac{v_z\sqrt{1-\frac{V^2}{c^2}}}{c-v_x \frac{V}{c}} = \dfrac{v_z\sqrt{1 - \frac{V^2}{c^2}}}{1 - \frac{v_x V}{c^2}}$
		
		$|v'|^2 = \dfrac{1}{\left(1 - \frac{v_x V}{c^2}\right)^2}\left[v_x^2 + v_y^2 \left(1 - \dfrac{V^2}{c^2}\right) + v_z^2 \left(1-\dfrac{V^2}{c^2}\right)\right] = \dfrac{|v|^2}{\left(1 - \frac{v_x V}{c^2}\right)^2} - \dfrac{V^2}{c^2} \dfrac{v_y^2  + v_z^2}{\left(1 - \frac{v_x V}{c^2}\right)^2}$
	\end{center}
	
	\begin{equation}
		|v'_1||v'_2| = \sqrt{\left(\dfrac{|v_1|^2}{\left(1 - \frac{v_{1x} V}{c^2}\right)^2} - \dfrac{V^2}{c^2} \dfrac{v_{1y}^2  + v_{1z}^2}{\left(1 - \frac{v_{1x} V}{c^2}\right)^2}\right)\left(\dfrac{|v_2|^2}{\left(1 - \frac{v_{2x} V}{c^2}\right)^2} - \dfrac{V^2}{c^2} \dfrac{v_{2y}^2  + v_{2z}^2}{\left(1 - \frac{v_{2x} V}{c^2}\right)^2}\right)}
	\end{equation}
	
	\begin{equation}
		\vec{v'_1} \cdot \vec{v'_2}  = \dfrac{(v_{1x} - V)(v_{2x} - V)}{\left(1 - \frac{v_{1x} V}{c^2}\right)\left(1 - \frac{v_{2x} V}{c^2}\right)} + \dfrac{v_{1y} v_{2y} \left(1 - \frac{V^2}{c^2}\right)}{\left(1 - \frac{v_{1x} V}{c^2}\right)\left(1 - \frac{v_{2x} V}{c^2}\right)} + \dfrac{v_{1z} v_{2z} \left(1 - \frac{V^2}{c^2}\right)}{\left(1 - \frac{v_{1x} V}{c^2}\right)\left(1 - \frac{v_{2x} V}{c^2}\right)}
	\end{equation}
	
	\begin{equation}
		\cos\alpha' = \dfrac{(v_{1x} - V)(v_{2x} - V) + \left(\vec{v_1}\cdot\vec{v_2} - v_{1x}v_{2x}\right) \left(1-\frac{V^2}{c^2}\right) }{\sqrt{\left(\left(1-\frac{V^2}{c^2}\right)|\vec{v_1}| + \frac{V^2}{c^2} v_{1x}^2 \right)\left(\left(1-\frac{V^2}{c^2}\right)|\vec{v_2}| + \frac{V^2}{c^2} v_{2x}^2 \right)}}
	\end{equation}
	
	\quad \textbf{Ответ:} $\cos\alpha' = \dfrac{(v_{1x} - V)(v_{2x} - V) + \left(\vec{v_1}\cdot\vec{v_2} - v_{1x}v_{2x}\right) \left(1-\frac{V^2}{c^2}\right) }{\sqrt{\left(\left(1-\frac{V^2}{c^2}\right)|\vec{v_1}|^2 + \frac{V^2}{c^2} v_{1x}^2 \right)\left(\left(1-\frac{V^2}{c^2}\right)|\vec{v_2}|^2 + \frac{V^2}{c^2} v_{2x}^2 \right)}}$
	...
	\quad \textbf{Решение.} Ионнизированный газ в однородном магнитном поле является однородной гиротропной средой, в которой диэлектрическая проницаемость является тензорной величиной вида:
	
	\begin{equation}
		\varepsilon_{ik} = \left(\begin{array}{ccc}
			\varepsilon_{\perp} & -i\varepsilon_{\alpha} & 0 \\
			i\varepsilon_{\alpha} & \varepsilon_{\perp} & 0 \\
			0 & 0 & \varepsilon_{\parallel} \\ 
		\end{array}\right)
	\end{equation}
	
	\quad Распишем уравнения Масвела: 
	...
	\begin{equation}
		...
		\end{cases}, \quad \begin{cases} 
			\vec{H} \equiv \vec{B} \\
			D_k = \varepsilon_{ki} E_i \\
		\end{cases}
	\end{equation}
	
	\quad Подставляем в уравнения разложение на плоские волны: 
	
	\begin{equation}
		\begin{cases}
			\vec{E} = \vec{E_0} e^{i(\omega t - \vec{k}\cdot\vec{r})} \\
			\vec{H} = \vec{H_0} e^{i(\omega t - \vec{k}\cdot\vec{r})} \\
		\end{cases} \implies \begin{cases}
			\vec{k} \times \vec{E} = \dfrac{\omega}{c} \vec{H} \\
			\vec{k} \times \vec{H} = -\dfrac{\omega}{c} \vec{D} \\
		\end{cases}
	\end{equation}
	
	\begin{equation}
		\vec{k} \times \left[\vec{k} \times \vec{E} \right] = -\dfrac{\omega^2}{k^2} \vec{D} \quad\Leftrightarrow \quad k_i k_m E_j \varepsilon_{ijk} \varepsilon_{mkn} \vec{e_n} = -\dfrac{\omega^2}{c^2} \varepsilon_{s\alpha} E_\alpha \vec{e_s}  
	\end{equation}
	
	\begin{center}
		$k_i k_m E_j \varepsilon_{ijk}\varepsilon_{nmk} \vec{e_n} = k_i k_m E_j \left(\delta_{in} \delta_{jm} - \delta_{im} \delta_{jn} \right) \vec{e_n} = \vec{k} \left(\vec{k} \cdot \vec{E}\right) - k^2 \vec{E} $
	\end{center}
	
	\begin{equation}
		\implies k^2 \vec{E} - \vec{k}\left(\vec{k}\cdot\vec{E}\right) = \dfrac{\omega^2}{c^2} \varepsilon_{s\alpha} E_\alpha \vec{e_s}
	\end{equation}
	
	\quad Учтём, что волна распостраняется вдоль магнитного поля т.е. волновой вектор имеет вид:
	
	\begin{center}
		$\vec{k} = \left(0, \quad 0, \quad k\right)^T$
	\end{center}	
	...
	\begin{equation}
		k^2\left(\begin{array}{c}
			...
			E_y \\
			E_z \\
		\end{array}\right) - kE_z\left(\begin{array}{c}
			0 \\
			0 \\
			k \\
		\end{array}\right) = \dfrac{\omega^2}{c^2} \left(\begin{array}{ccc}
		\varepsilon_{\perp} & -i\varepsilon_{\alpha} & 0 \\
		i\varepsilon_{\alpha} & \varepsilon_{\perp} & 0 \\
		0 & 0 & \varepsilon_{\parallel} \\ 
		\end{array}\right) \left(\begin{array}{c}
	...
	\begin{equation}
		\begin{cases}
			\left(k^2 -\dfrac{\omega^2}{c^2}\varepsilon_\perp\right)E_x + i\varepsilon_\alpha \dfrac{\omega^2}{c^2} E_y = 0 \\ 
			-i\varepsilon_\alpha\dfrac{\omega^2}{c^2} E_x + \left(k^2 - \dfrac{\omega^2}{c^2}\varepsilon_\perp\right) E_y = 0\\
		\end{cases}
	\end{equation}
	
	\quad Из условия существования нетривального решения системы (62):
	
	\begin{equation}
		\left(k^2 - \dfrac{\omega^2}{c^2}\varepsilon_\perp\right)^2 - \varepsilon_\alpha^2 \dfrac{\omega^4}{c^4} = 0 \Leftrightarrow \dfrac{k^2 c^2}{\omega^2} = \varepsilon_\perp \pm \varepsilon_\alpha
	\end{equation}
	
	\begin{equation}
		v_\varphi = \dfrac{c}{\sqrt{\varepsilon_\perp \pm \varepsilon_\alpha}}
	\end{equation}
	
	\quad \textbf{Ответ}: $v_\varphi = \dfrac{c}{\sqrt{\varepsilon_\perp \pm \varepsilon_\alpha}}$ 
	
	\quad \textbf{Задача 784.} Поток частиц с зарядом $e$ и скоростью $v \ll c$ рассеивается на абсолютно твёрдой сфере радиуса $a$. Найти эффективное излучение $d\varkappa_\omega$ в интервале частот $d\omega$. Чему равно полное эффективное излучение $\varkappa$?
	
	\quad \textbf{Решение.} Зададимся прицельным параметром:
	
	\begin{equation}
		\sin\alpha = \dfrac{s}{a}, \quad \theta = \pi - 2\alpha \quad \Leftrightarrow \quad s = a\cos\dfrac{\theta}{2}
	\end{equation}
	
	\quad Согласно выражениям из задачника: 
	
	\begin{equation}
		d\varkappa_\omega = 2\pi \displaystyle\int\limits_{0}^{\infty} d\Delta W sds, \quad d\Delta W = \dfrac{2}{3\pi c^3} e\Delta\vec{v^2} d\omega
	\end{equation}
	
	\quad Теперь, исходя из геометрии задачи, определим изменение скоростей:
	
	\begin{equation}
		\vec{p_2} - \vec{p_1} = \Delta \vec{p} \implies 2v\cos\left( \dfrac{\pi}{2} -  \dfrac{\theta}{2}\right) = m\Delta v \quad \Leftrightarrow \quad \Delta v = 2v\sin\dfrac{\theta}{2}
	\end{equation}
	
	\quad Таким образом, выразим дифференциальное сечение: 
	
	\begin{equation}
		d\varkappa_\omega =\displaystyle\int\limits_{0}^{\pi} \dfrac{8e^2v^2 a^2}{3 c^3}\sin^2 \dfrac{\theta}{2} \sin\theta d\theta d\omega =  \dfrac{4e^2v^2 a^2}{3 c^3} d\omega \displaystyle\int\limits_{0}^{\pi}  \left(1-\cos\theta\right) d\left(1 - \cos\theta\right) = \dfrac{4e^2v^2 a^2}{3c^3}d\omega
	\end{equation} 
	
	\begin{equation}
		\varkappa = \displaystyle\int\limits_{0}^{\varkappa} d\varkappa_\omega = \displaystyle\int\limits_{0}^{\infty} \dfrac{4e^2v^2 a^2}{3c^3}d\omega \to \infty
	\end{equation}
	
	\quad \textbf{Ответ}: $d\varkappa_\omega = \dfrac{4e^2v^2 a^2}{3c^3}d\omega$, $\varkappa \to \infty$
	
	\quad \textbf{Задача 681.} Кристаллическая решётка способна принимать импульс только дискретными порциями $\mathbf{q} = 2\pi\hbar\mathbf{g}$, где $\mathbf{g}$ — вектор обратной решётки. В случае кристаллической решётки, элементарная ячейка которой имеет форму прямоугольного параллелепипеда с ребрами $a_1$, $a_2$, $a_3$, вектор $\mathbf{g} = \left( \dfrac{n_1}{a_1}, \dfrac{n_2}{a_2}, \dfrac{n_3}{a_3} \right)$, где $n_1$, $n_2$, $n_3$ — любые целые числа. Считая, что кристалл, имеющий очень большую массу, не может принимать от частицы энергию, выяснить, какой характер будет иметь угловое распределение частиц, рассеиваемых на монокристалле.
	
	\quad \textbf{Решение.} Распишем рассеяние на плоской поверхности:
	
	\begin{equation}
		\vec{p_2} - \vec{p_1} = \vec{\Delta p} \quad \Leftrightarrow \quad 2p\sin\dfrac{\theta}{2} = \Delta p
	\end{equation}
	
	\quad Поскольку изменение импульса может принимать только фиксированные значения из условия задачи, то: 

	\begin{equation}
		\sin\dfrac{\theta}{2} = \dfrac{2\pi \hbar g}{2p}, \quad g = \dfrac{1}{a} \implies \sin\dfrac{\theta}{2} = \dfrac{\pi \hbar}{pa}
	\end{equation}
	
	\quad \textbf{Ответ}: $\sin\dfrac{\theta}{2} = \dfrac{\pi \hbar}{pa}$
	
	\quad \textbf{Задача 857.} Частицы среды имеют одинаковые по абсолютной величине скорости $v_0$, распределённые сферически симметрично, заряды $e$ и массы $m$. Вычислить среднюю силу $\mathbf{F}$, действующую на пробную частицу с зарядом $e'$ и массой $m'$, которая движется со скоростью $\mathbf{v}$.
	
	\quad \textbf{Решение.} Распишем уравнение движения частицы в поле движущихся частиц: 
	
	\begin{equation}
		m' \dfrac{d\vec{v}}{dt} = q' \vec{E} + \dfrac{q'}{c} \left[\vec{v} \times \vec{B}\right], \quad v \ll c
	\end{equation}
	
	\begin{equation}
		m' \dfrac{d\vec{v}}{dt} = q'\vec{E}, \quad \vec{E} = \dfrac{q\vec{r}(t)}{r^3}, \quad \vec{r}(t) = \vec{l_0} + \vec{v} t 
	\end{equation}
	
	\begin{equation}
		r^3 = \left(l_0^2 + v^2 t^2\right)^{\frac{3}{2}} \implies \Delta \vec{p} = \displaystyle\int\limits_{-\infty}^{\infty} \dfrac{qq' (\vec{l_0} + \vec{v}t) dt}{\left(l_0^2 + v^2 t^2\right)^{\frac{3}{2}}} = qq'\displaystyle\int\limits_{-\infty}^{\infty} \dfrac{\vec{l_0}dt}{\left(l_0^2 + v^2 t^2 \right)^{\frac{3}{2}} } = \dfrac{2qq'\vec{l_0}}{v l_0^2}
	\end{equation}
	
	\quad Изменение импульса характеризует силу, которая действует со стороны поля движущихся зарядов на пробную частицу. Усредняем силу по числу столкновений $N$:
	
	\begin{equation}
		dN = 2\pi nf(\vec{v_0}) d^3 v_0 v sds, \quad \left<\vec{F}\right> = \displaystyle\int \Delta \vec{p} dN 
	\end{equation}
	
	\quad где $f(\vec{v_0})$ - функция плотности вероятности, $n$ - концентрация, $s$ - прицельный параметр.
	
	\begin{equation}
		\left<\vec{F}\right> = 4\pi qq' n \displaystyle\int\limits_{-\infty}^{\infty} f(\vec{v_0}) d^3 \vec{v_0} \displaystyle\int\limits_{0}^{\infty} \dfrac{\vec{s}}{s}ds
	\end{equation}
	
	\quad Отсюда находим: 
	
	\begin{equation}
		\left<\vec{F}\right> = 4\pi q^2 q'^2\lambda \left(\dfrac{1}{m} - \dfrac{1}{m'}\right) \dfrac{n\vec{v}}{v^3}, \quad \forall v > v_0
	\end{equation}
	
	\quad \textbf{Ответ}: $\left<\vec{F}\right> = 4\pi q^2 q'^2\lambda \left(\dfrac{1}{m} - \dfrac{1}{m'}\right) \dfrac{n\vec{v}}{v^3}, \quad \forall v > v_0$
